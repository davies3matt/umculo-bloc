...
export default async (event, context) => {
  ...
  await Promise.all([
    ...
    ...users.map(async (user) => {
      ...
      const pendingGroups = invitedUser.pendingGroups
      ...
      if (!pendingUsers.includes(invitedUser.id)) {
        await update("Group", groupId, {
          pendingUsers: [...pendingUsers, ...[invitedUser.id]],
        })
        // add group id to user's pending groups
        return update("User", invitedUser.id, {
          pendingGroups: [...pendingGroups, ...[groupId]],
        })
      }
    }),
  ])
